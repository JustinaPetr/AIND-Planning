...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            # Initialize an action Node for current action
            node_a = PgNode_a(action)
            # Check if preconditions are satisfied
            if node_a.prenodes.issubset(self.s_levels[level]):
				#Add action node to the list of action nodes, make it a child of preceding level nodes and make prededing level nodes parents of this action node
                action_nodes.append(node_a)
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)
                    node_a.parents.add(node_s)
        # Add action level from the reachable action nodes
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            for node_s in node_a.effnodes:
				#Add the node to the state nodes. Make this node a child of preceding level node and make preceding level node a parent of this node.
                s_nodes.add(node_s)
                node_s.parents.add(node_a)
                node_a.children.add(node_s)
        # Create a  the state level with s_nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		# Check that node_a1's (+) effects aren't cancelled by node_a2's (-) effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		#Check that node_a1's (+) don't contradict with node_a2's (-) preconditions and vise versa.
        for eff in node_a1.action.effect_add:
        ...
        # Check that node_a1's (-)  don't conflict with node_a2's (+) preconditions and node_a2's (-)  don't contradict with node_a1's (+) preconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for prec_a1 in node_a1.parents:
            for prec_a2 in node_a2.parents:
                if prec_a1.is_mutex(prec_a2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for prec_s1 in node_s1.parents:
            for prec_s2 in node_s2.parents:
                if not prec_s1.is_mutex(prec_s2):
                    return False
